...
到这里，可以说将电机控制的原理讲的比较清楚了，后面的几步就是跟实际控制相关的了，接下来就是在实际工程中如何去使用和应用了。

### 四、电机控制问题

1、电流采样问题

​		在实际的电机控制中，需要得到三相的电流去进行控制，就会涉及电流采样的问题，根据基尔霍夫电流定律，在任意瞬间，同一节点流进和流出的电流相等，用数学表达式就是：
$$
I_a+I_b+I_c=0
$$
...
​		第二种方法是双电阻采样，在硬件电路设计中，可以设计双电阻采样，比如同一时刻采集A相和B相的电流，根据上面的公式可以计算出C相的电流。
...
​		硬件上只有一个采样电阻，但是为了得到三相的电流，需要在一个周期内采集两次电流，然后计算得到三相的电流，继而进行后面的计算。单电阻采样方案成本最低，但是在控制上要求会高一些。
